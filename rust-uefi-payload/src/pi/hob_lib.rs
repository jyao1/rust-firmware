...
use r_efi::efi;
use r_efi::efi::{AllocateType, MemoryType, PhysicalAddress, Status};
...
fn dump_allocation_hob(allocation_hob: &MemoryAllocation) {
    log!(
        ...
        allocation_hob.alloc_descriptor.memory_base_address
            + allocation_hob.alloc_descriptor.memory_length
            - 1,
    );
}
...
pub fn relocate_hob(hob: *const c_void) -> *mut c_void {
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(hob, new_hob_ptr, hob_total_size);
    }
