...
impl Image {
    pub fn load_image(
        ...
        let status = crate::efi::allocate_pool(
            ...
            size_of::<ImageInfo>() + device_path_size,
            &mut handle_address,
        );
        ...
        let device_path_buffer: *mut c_void =
            (handle_address as usize + size_of::<ImageInfo>()) as *mut c_void;
        unsafe {
        ...
        let status =
            crate::efi::allocate_pool(MemoryType::BootServicesData, image_size, &mut image_address);
        if status != Status::SUCCESS {
            log!("load_image - fail on allocate pool\n");
        ...
        handle.entry_point =
            peloader_load_image(image_address, image_size, source_buffer, source_size);
        ...
        let loaded_image_address: *mut c_void =
            (handle_address as usize + offset_of!(ImageInfo, loaded_image)) as *mut c_void;
        let mut loaded_image =
            unsafe { transmute::<*mut c_void, &mut LoadedImageProtocol>(loaded_image_address) };
        loaded_image.revision = r_efi::protocols::loaded_image::REVISION;
        loaded_image.parent_handle = parent_image_handle;
        loaded_image.system_table =
            unsafe { &mut crate::efi::ST as *mut r_efi::system::SystemTable };
    ...
    pub fn start_image(&mut self, image_handle: Handle) -> (Status, usize, *mut Char16) {
        let mut handle_address: *mut c_void = core::ptr::null_mut();
