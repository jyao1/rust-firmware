...
use r_efi::protocols::device_path::TYPE_END;
...
pub fn get_device_path_node_size(device_path: *mut DevicePathProtocol) -> usize {
    unsafe {
...
pub fn get_device_path_node_type(device_path: *mut DevicePathProtocol) -> u8 {
    unsafe { (*device_path).r#type }
}

pub fn get_device_path_node_sub_type(device_path: *mut DevicePathProtocol) -> u8 {
    unsafe { (*device_path).sub_type }
}

pub fn get_next_device_path_node(device_path: *mut DevicePathProtocol) -> *mut DevicePathProtocol {
    let size: usize = get_device_path_node_size(device_path);
    (device_path as usize + size) as *mut DevicePathProtocol
}

pub fn get_device_path_size(device_path: *mut DevicePathProtocol) -> usize {
    ...
    loop {
        ...
        if get_device_path_node_type(device_path_node) == r_efi::protocols::device_path::TYPE_END
            && get_device_path_node_sub_type(device_path_node)
                == r_efi::protocols::device_path::End::SUBTYPE_ENTIRE
        {
            break;
        }
...
pub fn dump_device_path(device_path: *mut DevicePathProtocol) {
    let size = get_device_path_node_size(device_path);
    crate::log!(
        "node_type: {} node_size: {} node_data:",
        get_device_path_node_type(device_path),
        get_device_path_node_size(device_path)
    );
    for i in 0..size {
        if i % 8 == 0 {
            crate::log!("\n");
...
pub fn print_device_path(device_path: *mut DevicePathProtocol) {
    let mut device_path_node = device_path;
    loop {
        // let node_size = get_device_path_node_size(device_path_node);
        // size = size + node_size;
...
pub fn get_file_path_media_device_path(device_path: *mut DevicePathProtocol) -> Option<*mut u16> {
    ...
    loop {
        ...
        if get_device_path_node_type(device_path_node) == r_efi::protocols::device_path::TYPE_MEDIA
            && get_device_path_node_sub_type(device_path_node)
                == r_efi_ext::protocols::device_path::Media::SUBTYPE_FILE_PATH
        {
