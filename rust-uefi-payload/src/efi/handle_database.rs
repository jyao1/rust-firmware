...
impl HandleDatabase {
    pub fn install_protocol(
        ...
        let protocol_struct =
            unsafe { transmute::<*mut ProtocolStruct, &mut ProtocolStruct>(cur_protocol_struct) };
    ...
    pub fn install_multiple_protocol(
        ...
        match status {
            Status::SUCCESS => {
                assert!(cur_handle != core::ptr::null_mut());
                let protocol_handle =
                    unsafe { transmute::<Handle, *mut ProtocolHandle>(cur_handle) };
                unsafe {
                    assert!((*protocol_handle).signature == HANDLE_SIGNATURE);
                }
                ...
                for index in 0..count {
                    let (status, protocol_struct) =
                        self.get_protocol(protocol_handle, unsafe { (*pair)[index].0 });
        ...
        for index in 0..count {
            ...
            let protocol_struct = unsafe {
                transmute::<*mut ProtocolStruct, &mut ProtocolStruct>(cur_protocol_struct)
            };
    ...
    fn locate_handle_count(&mut self, guid: *mut Guid) -> (Status, usize) {
    ...
    fn locate_handle_copy(
        ...
        for index in 0..self.protocol_handle_count {
            ...
            if status == Status::SUCCESS {
                ...
                unsafe {
                    (*handle_buffer)[count] = protocol_handle as Handle;
                }
                count = count + 1;
            }
        }
    ...
    pub fn locate_handle_buffer(&mut self, guid: *mut Guid) -> (Status, usize, *mut Handle) {
        ...
        self.locate_handle_copy(
            guid,
            handle_buffer_address,
            count * core::mem::size_of::<Handle>(),
        );
    ...
    pub fn handle_protocol(&mut self, handle: Handle, guid: *mut Guid) -> (Status, *mut c_void) {
    ...
    pub fn locate_protocol(&mut self, guid: *mut Guid) -> (Status, *mut c_void) {
        for index in 0..self.protocol_handle_count {
            let protocol_handle = &mut self.protocol_handle[index] as *mut ProtocolHandle;
            let (status, protocol_struct) = self.get_protocol(protocol_handle, guid);
            if status == Status::SUCCESS {
    ...
    fn get_new_protocol(
        ...
        unsafe {
            ...
            let protocol_struct =
                &mut (*protocol_handle).protocol_struct[(*protocol_handle).protocol_count];
            (*protocol_handle).protocol_count = (*protocol_handle).protocol_count + 1;

            protocol_struct.guid = Guid::from_fields(
                0x00000000,
                0x0000,
                0x0000,
                0x00,
        ...
        unsafe {
            ...
            for index in 0..(*protocol_handle).protocol_count {
                ...
                if *guid == guid_data {
                    return (
                        Status::SUCCESS,
                        &mut (*protocol_handle).protocol_struct[index],
                    );
                }
            }
        }
    ...
    fn get_new_handle(&mut self) -> (Status, Handle) {
        ...
        (
            Status::SUCCESS,
            protocol_handle as *mut ProtocolHandle as Handle,
        )
    }

    fn get_handle(&mut self, handle: Handle) -> (Status, Handle) {
