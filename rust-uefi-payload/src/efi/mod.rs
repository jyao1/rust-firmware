...
use r_efi::protocols::simple_text_input_ex::{KeyData, KeyNotifyFunction, KeyToggleState};
...
use crate::pi::hob::{
    Header, MemoryAllocation, ResourceDescription, HOB_TYPE_END_OF_HOB_LIST,
    HOB_TYPE_MEMORY_ALLOCATION, HOB_TYPE_RESOURCE_DESCRIPTOR, RESOURCE_SYSTEM_MEMORY,
};
...
pub fn print_guid(guid: *mut Guid) {
...
pub fn get_char16_size(message: *mut Char16, max_size: usize) -> usize {
    let mut i: usize = 0;
    loop {
        if (i >= max_size) {
            break;
...
pub fn print_char16(message: *mut Char16, max_size: usize) -> usize {
    let mut i: usize = 0;
    loop {
        if (i >= max_size) {
            break;
...
pub extern "win64" fn stdin_read_key_stroke(
    ...
    if false {
        ...
        stdout_output_string(
            unsafe { &mut STDOUT },
            &mut string as *mut [Char16; 8] as *mut u16,
        );
    }
...
// #[cfg(not(test))]
pub extern "win64" fn stdout_set_mode(
    _: *mut SimpleTextOutputProtocol,
    mode_number: usize,
) -> Status {
...
// #[cfg(not(test))]
pub extern "win64" fn stdout_set_attribute(
    _: *mut SimpleTextOutputProtocol,
    attribute: usize,
) -> Status {
...
// #[cfg(not(test))]
pub extern "win64" fn stdout_enable_cursor(
    _: *mut SimpleTextOutputProtocol,
    visible: Boolean,
) -> Status {
    Status::SUCCESS
...
pub extern "win64" fn get_variable(
    ...
    let guid_buffer: *mut [u8; 16] =
        unsafe { core::mem::transmute::<*mut Guid, *mut [u8; 16]>(var_guid) };
    ...
    let data_ptr: *mut c_void =
        unsafe { core::mem::transmute::<*mut [u8; MAX_VARIABLE_DATA], *mut c_void>(var_data) };
    unsafe {
        core::ptr::copy_nonoverlapping(data_ptr, data, var_size);
    }
...
pub extern "win64" fn set_variable(
    ...
    let data_buffer: *mut [u8; MAX_VARIABLE_DATA] =
        unsafe { core::mem::transmute::<*mut c_void, *mut [u8; MAX_VARIABLE_DATA]>(data) };
...
pub extern "win64" fn create_event(
    ...
    let (status, new_event) =
        EVENT
            .lock()
            .create_event(r#type, notify_tpl, notify_function, notify_context);
    crate::log!(
        "EFI_STUB: create_event - type:0x{:x} tpl:0x{:x} - status: {:?}\n",
        r#type,
        notify_tpl as usize,
        status
    );
...
pub extern "win64" fn install_protocol_interface(
    ...
    let (status, new_handle) =
        HANDLE_DATABASE
            .lock()
            .install_protocol(unsafe { *handle }, guid, interface);
...
pub extern "win64" fn handle_protocol(
    ...
    {
        crate::log!(
            "EFI_STUB - handle_protocol: {:?}, handle: {:?} - status {:x}, interface: {:?}\n",
            unsafe { *guid },
            handle,
            status.as_usize(),
            interface
...
pub extern "win64" fn locate_handle(
    ...
    let (status, final_buffer_size) =
        HANDLE_DATABASE
            .lock()
            .locate_handle(guid, input_buffer_size, buffer);
    ...
    match status {
        ...
        Status::NOT_FOUND => {}
        _ => {
            crate::log!("EFI_STUB: locate_handle error\n");
            return status;
        }
    }
...
pub extern "win64" fn locate_device_path(
    ...
    let mut tmp_device_path: *mut DevicePathProtocol =
        unsafe { *device_path as *mut DevicePathProtocol };
    ...
    let (status, handle_count, handle_buffer) =
        HANDLE_DATABASE.lock().locate_handle_buffer(protocol);
    ...
    for index in 0..handle_count {
        ...
        let (status, interface) = HANDLE_DATABASE.lock().handle_protocol(
            handle,
        ...
        let size =
            crate::efi::device_path::get_device_path_size(interface as *mut DevicePathProtocol) - 4;

        if (size as u64 <= source_size)
            && crate::efi::device_path::compare_device_path(
                interface as *mut DevicePathProtocol,
                source_path,
                size,
            )
        {
...
pub extern "win64" fn load_image(
    ...
    crate::log!(
        "EFI_STUB: load_image size is: {}, parent_image_handle: {:?}\n",
        source_size,
        parent_image_handle
    );
    ...
    if source_size == 0 {
        ...
        if let Some(filename) =
            crate::efi::device_path::get_file_path_media_device_path(device_path)
        {
            let mut name = [0u8; 512];
            char16_to_char8(filename, 256, &name[0] as *const u8 as *mut u8, 256);
            log!(
                "EFI_STUB: filename is {}\n",
            ...
            let status = locate_protocol(
                &efi::protocols::simple_file_system::PROTOCOL_GUID as *const efi::Guid
                    as *mut efi::Guid,
                handle,
                &mut fs_interface,
            );
            crate::log!(
                "EFI_STUB: simple_file_system protocol 0x{:p} status: {:x}\n",
                fs_interface,
                status.as_usize()
            );
            ...
            unsafe {
                status = ((*rootfile).open)(
                    rootfile,
                    &mut desfile as *mut *mut efi::protocols::file::Protocol,
                    filename as *mut efi::Char16,
                ...
                let mut buffer =
                    [0u8; core::mem::size_of::<crate::r_efi_ext::protocols::file::Info>() + 1024];
                let mut buffer_size: usize =
                    core::mem::size_of::<crate::r_efi_ext::protocols::file::Info>() + 1024;
                status = ((*desfile).get_info)(
                    desfile,
                    &mut efi::protocols::file::INFO_ID.clone() as *mut efi::Guid,
                    &mut buffer_size,
                    &mut buffer[0] as *mut u8 as *mut core::ffi::c_void,
                );
                if status.is_error() {
                    log!(
                        "EFI_STUB: load image get_info error 0x{:x}\n",
                        status.as_usize()
                    );
                }
                let desfile_info =
                    &buffer as *const u8 as *mut crate::r_efi_ext::protocols::file::Info;
                source_size = unsafe { (*desfile_info).file_size } as usize;
                log!("EFI_STUB: file size is: {:?}\n", source_size);

                source_buffer = core::ptr::null_mut();
                status = crate::efi::allocate_pool(
                ...
                if status.is_error() {
                    log!(
                        "EFI_STUB: load image read error 0x{:x}\n",
                        status.as_usize()
                    );
                }
            }
    ...
    crate::log!(
        "EFI_STUB: load_image done handle {:?} status 0x{:x}\n",
        new_image_handle,
        status.as_usize()
    );
    if status == Status::SUCCESS {
...
pub extern "win64" fn open_protocol(
    ...
    unsafe {
        *out_interface = core::ptr::null_mut();
    }
    let (status, interface) = HANDLE_DATABASE.lock().handle_protocol(handle, guid);
...
// #[cfg(not(test))]
pub extern "win64" fn locate_protocol(
    guid: *mut Guid,
    ...
    if status == Status::SUCCESS {
        ...
        crate::log!(
            "EFI_STUB - locate_protocol: {:?} failed, status: {:?}\n",
            unsafe { *guid },
            status
        );
    }
...
pub extern "win64" fn install_multiple_protocol_interfaces_real(
    ...
    let mut pair: [(*mut Guid, *mut c_void); 8] =
        [(core::ptr::null_mut(), core::ptr::null_mut()); 8];

    if guid1 == core::ptr::null_mut() {
        crate::log!(
            "EFI_STUB: install_multiple_protocol_interfaces_real - no GUID/Interface pair\n"
        );
        return Status::INVALID_PARAMETER;
    } else {
        count = 1;
    ...
    if guid_null != core::ptr::null_mut() {
        crate::log!(
            "EFI_STUB: install_multiple_protocol_interfaces_real - too many GUID/Interface pair\n"
        );
        return Status::UNSUPPORTED;
    }
    ...
    let (status, new_handle) =
        HANDLE_DATABASE
            .lock()
            .install_multiple_protocol(unsafe { *handle }, count, &mut pair);
    log!("status - {:?}\n", status);
    if status == Status::SUCCESS {
        unsafe {
            *handle = new_handle;
...
pub extern "win64" fn copy_mem(dest: *mut c_void, source: *mut c_void, size: usize) {
    unsafe {
        core::ptr::copy(source, dest, size);
    }
}
...
pub extern "win64" fn set_mem(buffer: *mut c_void, size: usize, val: u8) {
    unsafe {
        core::ptr::write_bytes(buffer, val, size);
    }
}
...
pub type InstallMultipleProtocolInterfacesFunc =
    extern "win64" fn(*mut Handle, *mut c_void, *mut c_void) -> r_efi::base::Status;
pub type UninstallMultipleProtocolInterfacesFunc =
    extern "win64" fn(*mut Handle, *mut c_void, *mut c_void) -> r_efi::base::Status;
...
fn dup_device_path(device_path: *mut c_void) -> *mut core::ffi::c_void {
    ...
    let device_path_size =
        crate::efi::device_path::get_device_path_size(device_path as *mut DevicePathProtocol);
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(device_path, device_path_buffer, device_path_size);
    }
...
pub fn enter_uefi(hob: *const c_void) -> ! {
    unsafe {
        ...
        let func_addr_ptr = unsafe {
            transmute::<&mut InstallMultipleProtocolInterfacesFunc, *mut usize>(
                &mut BS.install_multiple_protocol_interfaces,
            )
        };
        unsafe {
            *func_addr_ptr = install_multiple_protocol_interfaces_real as usize;
        }
        let func_addr_ptr = unsafe {
            transmute::<&mut UninstallMultipleProtocolInterfacesFunc, *mut usize>(
                &mut BS.uninstall_multiple_protocol_interfaces,
            )
        };
        unsafe {
            *func_addr_ptr = uninstall_multiple_protocol_interfaces_real as usize;
        }

        ST.number_of_table_entries = MAX_CONFIGURATION_TABLE;
        ST.configuration_table = &mut CT
            as *mut [r_efi::system::ConfigurationTable; MAX_CONFIGURATION_TABLE]
            as *mut r_efi::system::ConfigurationTable;
    }
    ...
    unsafe {
        crate::efi::init::initialize_console(
            &mut ST,
            &mut STDIN_EX as *mut SimpleTextInputExProtocol as *mut c_void,
        );
    }
        ...
        Some(pci_device) => {
            ...
            match part::find_efi_partition(&device) {
                Ok((start, end)) => {
                    ...
                    partition_end = end;
                    let efi_part_id = unsafe {
                        crate::efi::block::populate_block_wrappers(
                            &mut crate::efi::BLOCK_WRAPPERS,
                            &device,
                        )
                    };
                    ...
                    let status = crate::efi::install_protocol_interface(
                        ...
                        &mut r_efi::protocols::simple_file_system::PROTOCOL_GUID.clone()
                            as *mut Guid,
                    ...
                    let mut device_path_buffer: *mut core::ffi::c_void = dup_device_path(
                        &mut file_system_path.file_system_path_node.header
                            as *mut DevicePathProtocol as *mut c_void,
                    );
                    log!(
                        "device_path_buffer address: {:?}, device_path: {:?}\n",
                        device_path_buffer,
                        unsafe { *(device_path_buffer as *mut DevicePathProtocol) }
                    );
                    let status = crate::efi::install_protocol_interface(
                        &mut handle,
                        &mut r_efi::protocols::device_path::PROTOCOL_GUID.clone() as *mut Guid,
