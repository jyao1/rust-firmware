    ...
    loop {
        ...
        match header.r#type {
            HOB_TYPE_RESOURCE_DESCRIPTOR => {
                let resource_hob =
                    unsafe { transmute::<*const Header, &ResourceDescription>(hob_header) };
    ...
    loop {
        ...
        match header.r#type {
            HOB_TYPE_MEMORY_ALLOCATION => {
                let allocation_hob =
                    unsafe { transmute::<*const Header, &MemoryAllocation>(hob_header) };
...
pub fn initialize_variable() {
    let mut var_name: [Char16; 13] = [
        0x50, 0x6c, 0x61, 0x74, 0x66, 0x6F, 0x72, 0x6d, 0x4c, 0x61, 0x6e, 0x67, 0x00,
    ]; // L"PlatformLang"
...
pub fn initialize_fs() {
    ...
    let efi_part_id = unsafe {
        crate::efi::block::populate_block_wrappers(&mut crate::efi::BLOCK_WRAPPERS, &device)
    };
    ...
    let mut device_path_buffer: *mut core::ffi::c_void = dup_device_path(
        &mut file_system_path.file_system_path_node.header as *mut DevicePathProtocol
            as *mut c_void,
    );
    log!(
