...
static mut SET_MODE_STRING: [u16; SET_MODE_STRING_SIZE] =
    [ESC, '[' as u16, '=' as u16, '3' as u16, 'h' as u16, 0];
static mut SET_ATTRIBUTE_STRING: [u16; SET_ATTRIBUTE_STRING_SIZE] = [
    ESC, '[' as u16, '0' as u16, 'm' as u16, ESC, '[' as u16, '4' as u16, '0' as u16, 'm' as u16,
    ESC, '[' as u16, '4' as u16, '0' as u16, 'm' as u16, 0,
];
static mut CLEAR_SCREEN_STRING: [u16; CLEAR_SCREEN_STRING_SIZE] =
    [ESC, '[' as u16, '2' as u16, 'J' as u16, 0];
static mut SET_CURSOR_POSITION_STRING: [u16; SET_CURSOR_POSITION_STRING_SIZE] = [
    ESC, '[' as u16, '0' as u16, '0' as u16, ';' as u16, '0' as u16, '0' as u16, 'H' as u16, 0,
];
...
impl ConOut {
    ...
    pub fn output_string(&mut self, message: *mut u16) {
        let mode: *mut SimpleTextOutputMode =
            self.mode_ptr as *mut c_void as *mut SimpleTextOutputMode;
        let max_column = 80;
        let mut max_row = 25;
        unsafe {
            if (*mode).mode == 1 {
    ...
    pub fn set_cursor_position(&mut self, column: usize, row: usize) {
        unsafe {
            ...
            SET_CURSOR_POSITION_STRING[COLUMN_OFFSET + 0] =
                ('0' as usize + ((column + 1) / 10)) as u16;
            SET_CURSOR_POSITION_STRING[COLUMN_OFFSET + 1] =
                ('0' as usize + ((column + 1) % 10)) as u16;

            if (OUTPUT_ESC) {
                self.output_string(
                    &mut SET_CURSOR_POSITION_STRING as *mut [u16; SET_CURSOR_POSITION_STRING_SIZE]
                        as *mut u16,
                );
            }
        }
    ...
    pub fn set_attribute(&mut self, attribute: usize) {
        ...
        unsafe {
            ...
            SET_ATTRIBUTE_STRING[FOREGROUND_CONTROL_OFFSET + 0] =
                ('0' as usize + (foreground_control / 10)) as u16;
            SET_ATTRIBUTE_STRING[FOREGROUND_CONTROL_OFFSET + 1] =
                ('0' as usize + (foreground_control % 10)) as u16;
            SET_ATTRIBUTE_STRING[BACKGROUND_CONTROL_OFFSET + 0] =
                ('0' as usize + (background_control / 10)) as u16;
            SET_ATTRIBUTE_STRING[BACKGROUND_CONTROL_OFFSET + 1] =
                ('0' as usize + (background_control % 10)) as u16;

            if (OUTPUT_ESC) {
                self.output_string(
                    &mut SET_ATTRIBUTE_STRING as *mut [u16; SET_ATTRIBUTE_STRING_SIZE] as *mut u16,
                );
            }
        }
        unsafe {
    ...
    pub fn clear_screen(&mut self) {
        unsafe {
            if (OUTPUT_ESC) {
                self.output_string(
                    &mut CLEAR_SCREEN_STRING as *mut [u16; CLEAR_SCREEN_STRING_SIZE] as *mut u16,
                );
            }
        }
    ...
    pub fn set_mode(&mut self, mode_number: usize) -> Status {
        ...
        unsafe {
            if (OUTPUT_ESC) {
                self.output_string(
                    &mut SET_MODE_STRING as *mut [u16; SET_MODE_STRING_SIZE] as *mut u16,
                );
            }
        }
