...
impl fmt::Debug for FspInfoHeader {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        f.debug_struct("FspInfoHeader")
            ...
            .field("header_revision", &format_args!("0x{:x}", self.header_revision))
            .field("image_revision", &format_args!("0x{:x}", self.image_revision))
            .field("image_id", &format_args!(
                "{}{}{}{}{}{}{}{}",
            ...
            .field("temp_ram_init_entry_offset", &format_args!("0x{:x}", self.temp_ram_init_entry_offset))
            .field("notify_phase_entry_offset", &format_args!("0x{:x}", self.notify_phase_entry_offset))
            .field("fsp_memory_init_entry_offset", &format_args!("0x{:x}", self.fsp_memory_init_entry_offset))
            .field("temp_ram_exit_entry_offset", &format_args!("0x{:x}", self.temp_ram_exit_entry_offset))
            .field("fsp_silicon_init_entry_offset", &format_args!("0x{:x}", self.fsp_silicon_init_entry_offset))
            .finish()
    }
}
...
mod tests {
    ...
    fn fsp_info_header_offset() {
        ...
        use scroll::Pread;
        let fsp_info_header_bytes = &include_bytes!("../fsp_bins/Qemu/Rebase/FspRel_T_FFFC5000.raw")[..];
        let fsp_info_header = fsp_info_header_bytes.pread::<FspInfoHeader>(FSP_INFO_HEADER_OFF).unwrap();
